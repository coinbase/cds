...
2. **curl** if MCP is not configured, fails, or returns errors:
   - **Index:** `https://cds.cbhq.net/llms/web/routes.txt` or
     `https://cds.cbhq.net/llms/mobile/routes.txt`
   - **Page:** `https://cds.cbhq.net/llms/<route>` where `<route>` matches the index (e.g.
