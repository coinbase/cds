...
6. Imports come from `@cbhq/cds-web` for web or `@cbhq/cds-mobile` for mobile, using the documented component path instead of guessing.
7. Charts use `@cbhq/cds-web-visualization` or `@cbhq/cds-mobile-visualization` when CDS provides the needed visualization primitive.
...
2. Prefer `@cbhq/cds-web-visualization` on web and `@cbhq/cds-mobile-visualization` on mobile when CDS provides the chart or primitive.
...
- For standard web UI, use `@cbhq/cds-web`.
- For standard mobile UI, use `@cbhq/cds-mobile`.
- For web charts and data visualization, use `@cbhq/cds-web-visualization`.
- For mobile charts and data visualization, use `@cbhq/cds-mobile-visualization`.
...
The CDS MCP server exposes visualization and illustration docs through the same route listing as other CDS components. Do not assume that because a route is listed under `mobile/components/...` or `web/components/...` it should be imported from `@cbhq/cds-mobile` or `@cbhq/cds-web`. Read the doc and use the documented import path.
