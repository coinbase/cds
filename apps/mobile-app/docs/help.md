...
- `rm -rf apps/mobile-app/android` from root when you're done! Leaving the `android` directory will impact future builds.
...
- `rm -rf apps/mobile-app/ios apps/mobile-app/.expo` from root when you're done! Leaving the `ios` directory will impact future builds.
...
- `rm -rf apps/mobile-app/ios apps/mobile-app/.expo`
...
Look up the required version in `apps/mobile-app/package.json`.
...
7. No development build (com.anonymous.mobile-app) for this project is installed. Please make and install development build on the device first.

This error occurs because mobile-app/ios directory was present at the time of launching the build onto a simulator. This interferes with expo's naming of the app on the device.
