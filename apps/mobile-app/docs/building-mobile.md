...
- Any native dependency change in `apps/mobile-app/package.json` or `packages/mobile/package.json`
- Any change to native Expo config or build tooling

> JS-only changes do not require a full rebuild — use `yarn nx run mobile-app:patch-bundle-ios` / `patch-bundle-android` to swap the JS bundle into the existing prebuild instead.
...
You can create other build types using [app.json](/apps/mobile-app/app.json) and [project.json](/apps/mobile-app/project.json).

Create a new config in [project.json](/apps/mobile-app/project.json) `targets.build.configurations`. The key will become your new command for `yarn nx run mobile-app:build:<your key>`.
