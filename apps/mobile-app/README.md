...
| Command                                                 | Description                                                                             |
| ------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `yarn nx run mobile-app:ios`                             | Build (if needed), install, launch, and start Metro — full dev loop for iOS (debug)     |
| `yarn nx run mobile-app:ios --configuration=release`     | Install and launch the release build artifact (no Metro)                                |
| `yarn nx run mobile-app:android`                         | Build (if needed), install, launch, and start Metro — full dev loop for Android (debug) |
| `yarn nx run mobile-app:android --configuration=release` | Install and launch the release build artifact (no Metro)                                |
| `yarn nx run mobile-app:start`                           | Start Metro bundler only (assumes app is already installed)                             |
| `yarn nx run mobile-app:build --configuration=<config>`  | Compile the native app and archive to a tarball in `prebuilds/`                         |
| `yarn nx run mobile-app:launch --configuration=<config>` | Install + launch an existing build artifact on a simulator/emulator                     |
| `yarn nx run mobile-app:patch-bundle-ios`                | Swap the JS bundle inside the committed iOS Release prebuild — used by visreg CI        |
| `yarn nx run mobile-app:patch-bundle-android`            | Swap the JS bundle inside the committed Android Release prebuild — used by visreg CI    |
| `yarn nx run mobile-app:validate`                        | Check Expo dependency versions for compatibility                                        |
| `yarn nx run mobile-app:lint`                            | Lint the app source                                                                     |
| `yarn nx run mobile-app:typecheck`                       | Type-check the app source                                                               |

## Build configurations

| Configuration        | Platform | Profile | Target    | Output                                      |
| -------------------- | -------- | ------- | --------- | ------------------------------------------- |
| `ios-debug`          | iOS      | Debug   | Simulator | `prebuilds/ios-debug/mobileapp.tar.gz`       |
| `ios-release`        | iOS      | Release | Simulator | `prebuilds/ios-release/mobileapp.tar.gz`     |
| `ios-debug-device`   | iOS      | Debug   | Device    | `prebuilds/ios-debug-device/mobileapp.ipa`   |
| `ios-release-device` | iOS      | Release | Device    | `prebuilds/ios-release-device/mobileapp.ipa` |
| `android-debug`      | Android  | Debug   | Emulator  | `prebuilds/android-debug/mobileapp.apk`      |
| `android-release`    | Android  | Release | Emulator  | `prebuilds/android-release/mobileapp.apk`    |
...
yarn nx run mobile-app:build --configuration=ios-debug

# Then commit the updated tarballs
git add apps/mobile-app/prebuilds/
git commit -m "chore: update mobile-app prebuilds"
```
...
1. Extracts `prebuilds/ios-release/mobileapp.tar.gz` → `prebuilds/ios-release/mobileapp.app`
...
1. **Run the app**:
   ...
   This will:
   - Build the app if no artifact exists at `prebuilds/ios-debug/mobileapp.tar.gz`
...
2. **Run the app**:
   ...
   This will:
   - Build the APK if no artifact exists at `prebuilds/android-debug/mobileapp.apk`
