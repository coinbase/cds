...
- `PlaygroundContent` renders CDS components styled with the matched color:
  ...
  - `Card` with `LineChart` — avatar circle + line chart using the matched color as
    the primary series color (via `@cbhq/cds-web-visualization`).

## External Dependencies

| Package                           | Used For                                                  |
| --------------------------------- | --------------------------------------------------------- |
| `@cbhq/cds-web`               | All UI components (Button, Card, Tag, TextInput, etc.)    |
| `@cbhq/cds-web-visualization` | `LineChart`, `Scrubber`, `SolidLine` in playground        |
| `@cbhq/cds-common`            | `ThemeVars` types for spectrum color tokens               |
