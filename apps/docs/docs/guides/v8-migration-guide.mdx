...
Before diving into the breaking changes, we highly recommend starting with our automated migration script. The `@cbhq/cds-migrator` package handles many of the repetitive transformations automatically, saving you significant time and effort.
...
**Alternative for `cx` function:** If you're only using the `cx` function from `@linaria/core`, you can import it directly from `@cbhq/cds-web` instead of adding the dependency:
...
```tsx
// ❌ Before (v7)
import { borderRadius, colors } from '@cbhq/cds-mobile/tokens';
...
```tsx
// ❌ Before (v7)
import { deviceBreakpoints, deviceMqs } from '@cbhq/cds-web/layout/breakpoints';

// ✅ After (v8)
import { breakpoints, media } from '@cbhq/cds-web/styles/media';
```
...
// ✅ After (v8)
import { TextProps, TextDefaultElement } from '@cbhq/cds-web/typography/Text';
import { BoxProps, BoxDefaultElement } from '@cbhq/cds-web/layout/Box';
...
Yes! We provide the `@cbhq/cds-migrator` package to assist with automated migration. For detailed instructions, refer to the [Migrator Guide](https://github.com/coinbase/cds-staging/blob/master/packages/migrator/README.md).
...
**Note:** Code examples may show platform-specific import paths (e.g., `@cbhq/cds-web` or `@cbhq/cds-mobile`). Adjust the import paths based on your target platform.
...
```tsx
// ❌ Before (v7)
import { DevicePreferencesProvider } from '@cbhq/cds-mobile/system';
...
2. Add `import { MediaQueryProvider } from '@cbhq/cds-web/system/MediaQueryProvider'`
3. Replace `BreakpointsProvider` with `MediaQueryProvider`
4. Add the `defaultValues` prop if needed

```tsx
// ❌ Before (v7)
import { BreakpointsProvider } from '@cbhq/cds-web/system/BreakpointsProvider';
...
```tsx
// ❌ Before (v7)
import { RootSpectrumProvider } from '@cbhq/cds-mobile/system';
...
```tsx
// ❌ Before (v7)
import { DarkModeProvider, LightModeProvider } from '@cbhq/cds-mobile/system';
...
```tsx
// ❌ Before (v7)
import { ElevationConfigsProvider } from '@cbhq/cds-mobile/system';
...
```tsx
import { ThemeProvider } from '@cbhq/cds-web/system/ThemeProvider';
import { coinbaseDenseTheme } from '@cbhq/cds-web/themes/coinbaseDenseTheme';
...
1. Update the import path: `import { useThemeProviderStyles } from '@cbhq/cds-web/system/ThemeProvider';`
...
// ✅ After (v8)
import { useThemeProviderStyles } from '@cbhq/cds-web/system/ThemeProvider';
...
```tsx
// ❌ Before (v7)
import { paletteAliasToRgbaString } from '@cbhq/cds-common/palette/paletteAlias';
...
1. Remove import: `import { InteractableContent } from '@cbhq/cds-web/system';`
2. Import Interactable: `import { Interactable } from '@cbhq/cds-web/system';`
...
```tsx
// ❌ Before (v7)
import { overflowClassName } from '@cbhq/cds-web/cells/Cell';
...
```tsx
// ❌ Before (v7)
import { BoxElement } from '@cbhq/cds-web/layout/Box';
type MyProps = { as?: BoxElement };

// ✅ After (v8) - Option 1
import { BoxDefaultElement } from '@cbhq/cds-web/layout/Box';
...
```tsx
// ❌ Before (v7)
import { TextProps } from '@cbhq/cds-web/typography/Text';
...
// ✅ After (v8)
import { LinkProps, LinkDefaultElement } from '@cbhq/cds-web/typography/Link';
...
```tsx
// ❌ Before (v7)
import { HTMLNonHeadingTextTags } from '@cbhq/cds-web/typography';
...
```tsx
// ❌ Before (v7)
import { Overflow } from '@cbhq/cds-web/types';
...
```tsx
// ❌ Before (v7)
import { IconPixelSize } from '@cbhq/cds-common/types/IconSize';
...
```tsx
// ❌ Before (v7)
import { GapSpacing } from '@cbhq/cds-common/types/TooltipBaseProps';
...
```tsx
// ❌ Before (v7)
import { useIconSize } from '@cbhq/cds-web/hooks/useIconSize';
...
```tsx
// ❌ Before (v7)
import { useAvatarSize } from '@cbhq/cds-mobile/hooks/useAvatarSize';
