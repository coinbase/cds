...
- **Follow CDS best practices** — Prefer CDS style props and design tokens over raw CSS values, use semantic color tokens, and apply proper spacing and typography tokens.
...
- **Validate import paths** — Ensure all CDS imports use valid package export paths, preventing build errors from incorrect or guessed import paths.
...
:::tip
Skills work alongside the CDS MCP Server. For the best AI-assisted development experience, install both the MCP server and the agent skills.
:::
