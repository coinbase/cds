import { MDXSection } from '@site/src/components/page/MDXSection';
import { MDXArticle } from '@site/src/components/page/MDXArticle';

<MDXSection>
<MDXArticle>
## Prerequisites

Before you begin, make sure your development environment meets the following requirements:

- **Node.js**: Version 18 or later.
- **React**: Version 18 or later.

**For web projects, ensure your build system supports importing CSS files.**

</MDXArticle>
</MDXSection>

<MDXSection>
<MDXArticle>
## Installation

To install the CDS library for React web applications, run the following command:

```bash
npm install @cbhq/cds-web
```

Alternatively, if you are using Yarn:

```bash
yarn add @cbhq/cds-web
```

</MDXArticle>
</MDXSection>

<MDXSection>
<MDXArticle>
## Getting started
</MDXArticle>
<MDXArticle>
### 1. Import global styles

Some global and icon styles are required for components to render correctly. Import these styles near your application entry point.

```tsx
import '@cbhq/cds-icons/fonts/web/icon-font.css';
import '@cbhq/cds-web/globalStyles';
```

[See the `globalStyles` source code here &rarr;](https://github.com/coinbase/cds-staging/blob/master/packages/web/src/styles/global.ts)

If you are using the CDS `defaultTheme` you should also import the default font styles.

```tsx
import '@cbhq/cds-web/defaultFontStyles';
```

</MDXArticle>
<MDXArticle>

### 2. Render a ThemeProvider and MediaQueryProvider

Render a ThemeProvider at the root of your application, and pass the `theme` and `activeColorScheme`.

Render a MediaQueryProvider for components that use the `useMediaQuery` hook.

```tsx
import { ThemeProvider, MediaQueryProvider } from '@cbhq/cds-web/system';
import { defaultTheme } from '@cbhq/cds-web/themes/defaultTheme';
import App from './App';

const Index = () => {
  return (
    <MediaQueryProvider>
      <ThemeProvider theme={defaultTheme} activeColorScheme="light">
        <App />
      </ThemeProvider>
    </MediaQueryProvider>
  );
};

export default Index;
```

:::tip
The MediaQueryProvider prevents issues with `window.matchMedia()` in SSR environments. [Read more here](/components/other/MediaQueryProvider#server-side-rendering)
:::

</MDXArticle>
<MDXArticle>
### 3. Verify the installation

Try importing and rendering a simple CDS component.

```tsx
import { Button } from '@cbhq/cds-web/buttons';

const Test = () => (
  <Button>Click Me</Button>;
)
```

</MDXArticle>
</MDXSection>

<MDXSection>
<MDXArticle>

## Example implementation

Here's an example React DOM app using the `defaultTheme`.

```tsx
import '@cbhq/cds-icons/fonts/web/icon-font.css';
import '@cbhq/cds-web/defaultFontStyles';
import '@cbhq/cds-web/globalStyles';
import { createRoot } from 'react-dom/client';
import { ThemeProvider, MediaQueryProvider } from '@cbhq/cds-web/system';
import { defaultTheme } from '@cbhq/cds-web/themes/defaultTheme';
import App from './App';

const root = createRoot(document.getElementById('root'));

root.render(
  <MediaQueryProvider>
    <ThemeProvider theme={defaultTheme} activeColorScheme="light">
      <App />
    </ThemeProvider>
  </MediaQueryProvider>,
);
```

</MDXArticle>
</MDXSection>

<MDXSection>
<MDXArticle>
## Next steps

Learn how to customize CDS's appearance.

[See the theming docs here &rarr;](/getting-started/theming)

</MDXArticle>
</MDXSection>
