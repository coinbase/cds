...
```tsx
<NumberTicker value={12345.67} format={{ minimumFractionDigits: 2, maximumFractionDigits: 2 }} />
```
...
Note: Non-tabular numbers are currently not supported in mobile. The `tabularNumbers` setting should not be disabled at this point.

Also note: NumberTicker forwards all Text props, but only character-level typographic props (e.g., `font`, `fontFamily`, `fontSize`, `fontWeight`, `lineHeight`, `tabularNumbers`, `color`) are meaningful for its per-digit rendering. Layout or container-level props (e.g., `justifyContent`, `alignItems`) may have no effect and can cause unexpected behavior—use them judiciously.
...
## Prefix And Suffix (Text And React Nodes)
...
function PrefixAndSuffix() {
  ...
  const iconPrefixes = [
    ...
    <Icon key="arrowDown2" name="arrowDown" size="l" />,
  ];
  const iconSuffixes = [
    <Icon key="arrowDown" name="arrowDown" size="l" />,
    <Icon key="arrowUp" name="arrowUp" size="l" />,
...
Note: We do not support overriding text styles (color, font, etc.) for individual number sections in mobile. Text styles are applied globally within `NumberTicker`.
...
function Subscript() {
  ...
  return (
    <VStack gap={1}>
      ...
      <NumberTicker enableSubscriptNotation font="display3" format={format} value={value} />
      <Button onPress={onNext}>Next</Button>
    </VStack>
  );
}
  ...
  return (
    <VStack gap={1}>
      <NumberTicker
        ...
        styles={{ suffix: { position: 'relative', top: 6, opacity: 0.5 } }}
        suffix={suffix}
...
function StatisticsExample() {
  const [views, setViews] = React.useState(1234567);
  const [likes, setLikes] = React.useState(89432);
  const [shares, setShares] = React.useState(12789);
  const [downloads, setDownloads] = React.useState(567890);
