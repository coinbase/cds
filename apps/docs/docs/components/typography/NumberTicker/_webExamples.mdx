...
function Example() {
  ...
  return (
    <VStack gap={3}>
      <NumberTicker value={values[valIdx]} font="display3" />
      <Button
        onClick={() => {
...
## Responsive Font And Tabular Numbers

Use responsive typography tokens and toggle tabular numbers for aligned digits.
...
- Type: `{ y?: Transition; color?: Transition }`. `Transition` is the framer-motion type
- Optional `type`: `'tween' | 'spring' | 'inertia'` on each property; when omitted, it defaults to `'tween'`
...
<VStack gap={2}>
  ...
  <NumberTicker as="h1" value={12345.67} format={{ style: 'currency', currency: 'USD' }} />
  <NumberTicker as="p" value={12345.67} format={{ style: 'currency', currency: 'USD' }} />
</VStack>
```
...
Render a fully formatted string while still driving animations and color pulse from `value`.
...
A few practical demos that combine formatting, animation, and interactivity.
