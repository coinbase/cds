...
- Type: `{ y?: WithTimingConfig | WithSpringConfig; color?: WithTimingConfig | WithSpringConfig }`
- Optional `type`: `'timing' | 'spring'` on `y` and `color`; when omitted, defaults to `'timing'`
- Default: `{ y: { duration: durations.moderate3, easing: Easing.bezier(...curves.global) }, color: { duration: durations.slow4, easing: Easing.bezier(...curves.global) } }`
...
RollingNumber (mobile) is composed of small, swappable subcomponents and exposes granular style hooks for each section of the number. Use these to customize structure and styling or to plug in your own components.

### Subcomponents

- **RollingNumberMaskComponent**: Clips/scopes the animated digit content (container around visible number sections).
- **RollingNumberNodeSectionComponent**: Wraps ReactNode `prefix` and `suffix` props you pass to RollingNumber.
- **RollingNumberNumberSectionComponent**: Renders the formatted number parts produced by `Intl.NumberFormat` as four sections: `i18nPrefix`, `integer`, `fraction`, `i18nSuffix`.
- **RollingNumberDigitComponent**: Per-digit animated scroller (uses Reanimated for the roll animation).
- **RollingNumberSymbolComponent**: Renders non-digit characters (group separators, decimal, currency symbols, literals, etc.).
...
<RollingNumber
  ...
  RollingNumberNodeSectionComponent={MyNodeSection}
  RollingNumberNumberSectionComponent={MyNumberSection}
...
- **formattedNumberSection**: Container around the four i18n sections
...
- **prefix**: Your `prefix` prop (ReactNode) wrapper
- **suffix**: Your `suffix` prop (ReactNode) wrapper
- **text**: Global text style applied to digits/symbols/prefix/suffix (mobile only)
...
RollingNumber (root: HStack)
  ├── invisibleMeasuredDigits <Text x10>  (offscreen, used for measurements)
  ├── screenReaderOnly <Text accessibilityLiveRegion>  (hidden a11y text)
  └── HStack (visibleContent)
       ├── NodeSection (prefix)               ← your ReactNode prefix
       ├── HStack (formattedNumberSection)
       │    ├── NumberSection (i18nPrefix)
       │    ├── NumberSection (integer)
       │    ├── NumberSection (fraction)
       │    └── NumberSection (i18nSuffix)
       └── NodeSection (suffix)               ← your ReactNode suffix
```

Per-digit rendering inside a NumberSection (mobile translates the digit container so the active digit aligns with the top of the mask):
...
NumberSection
  ...
  └── Digit(s)
       └── Mask (RollingNumberMaskComponent)
            └── Animated.View (digit container)
                 ├── non-active digits above (positioned)
                 ├── active digit (top-aligned in mask)
