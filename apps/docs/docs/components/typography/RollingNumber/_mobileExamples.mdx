...
```tsx
<RollingNumber value={12345.67} format={{ minimumFractionDigits: 2, maximumFractionDigits: 2 }} />
```
...
Note: We do not support overriding text styles (color, font, etc.) for individual number sections in mobile. Text styles are applied globally within `RollingNumber`.
