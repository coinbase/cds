...
function Example() {
  ...
  return (
    <VStack gap={1}>
      ...
      <Button onClick={() => setIdx((i) => (i + 1) % values.length)}>Next</Button>
    </VStack>
  );
}
```
...
- **RollingNumberDigitComponent**: Per-digit animated scroller (handles the roll animation).
...
Use `styles` to inline style specific parts (same keys as `classNames` plus `root` and `visibleContent`):

- **root**, **visibleContent**, **formattedNumberSection**, **i18nPrefix**, **integer**, **fraction**, **i18nSuffix**, **prefix**, **suffix**
