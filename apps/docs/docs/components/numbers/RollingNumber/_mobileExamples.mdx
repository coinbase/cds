...
- **RollingNumberAffixSectionComponent**: Wraps ReactNode `prefix` and `suffix` props you pass to RollingNumber.
- **RollingNumberValueSectionComponent**: Renders the formatted number parts produced by `Intl.NumberFormat` as four sections: `i18nPrefix`, `integer`, `fraction`, `i18nSuffix`.
