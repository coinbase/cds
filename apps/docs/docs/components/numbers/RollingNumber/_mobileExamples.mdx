...
`styles.text` is the **only** style target capable of setting the default text color on mobile. It is explicitly threaded to every text element in RollingNumber (digits, symbols, prefix text, suffix text), acting as a manual substitute for CSS cascade.

Customizing the base color in this way is fully compatible with the `colorPulseOnUpdate` animation.

::::tip Web equivalent
On web, color cascades naturally through the DOM tree. This affords many ways to customize the default color, even on different elements. See the web docs for the full specificity hierarchy.
::::
  ...
  return (
    <VStack gap={2}>
      ...
      <RollingNumber
        ...
        value={price}
      />
      <Text font="label1">With color pulse</Text>
      <RollingNumber
        colorPulseOnUpdate
        font="title1"
        format={{ style: 'currency', currency: 'USD' }}
