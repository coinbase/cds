...
`colorPulseOnUpdate` is a semantic, token-driven effect and only works with the design-token `color` prop. Setting a custom color via `styles.text` **nullifies the pulse** — the number renders in your custom color and does not animate on change.
:::
