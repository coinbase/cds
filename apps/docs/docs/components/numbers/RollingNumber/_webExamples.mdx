...
To apply a non-token color (e.g. a hex string or CSS custom property), set `color` via an inline style or custom CSS. Any of these apply to the whole number:
...
`colorPulseOnUpdate` is a semantic, token-driven effect and only works with the design-token `color` prop. Setting a custom color through any of the inline-style targets above **nullifies the pulse** — the number renders in your custom color and does not animate on change.
:::

If more than one of the above sets `color`, the winner follows normal CSS specificity: `styles.text` (inline on the text) beats `styles.visibleContent`, which beats `styles.root`, which beats `style` (spread order).
