...
To apply a non-token color (e.g. a hex string or CSS custom property), set `color` via an inline style, `className`, or custom CSS. Any of these apply to the whole number:

- `style={{ color: '...' }}`
- `styles={{ root: { color: '...' } }}`
- `styles={{ visibleContent: { color: '...' } }}`
- `styles={{ text: { color: '...' } }}`
- a `className` targeting the component or its `classNames` slots

:::warning Custom colors and colorPulseOnUpdate
`colorPulseOnUpdate` is a semantic, token-driven effect and only works with the design-token `color` prop. It **will not work** alongside a custom color — combining a custom color with `colorPulseOnUpdate` is unsupported. Use one or the other.
:::

If more than one target sets `color`, the winner follows normal CSS specificity: `styles.text` (inline on the text) beats `styles.visibleContent`, which beats `styles.root`, which beats `style` (spread order).
...
Section-level targets (`styles.prefix`, `styles.suffix`, `styles.i18nPrefix`, `styles.i18nSuffix`, `styles.integer`, `styles.fraction`) color only their own section. That section renders statically, while the rest of the number can still pulse when `colorPulseOnUpdate` is enabled with a token `color`. In the example below the `$` stays green while the digits pulse.
