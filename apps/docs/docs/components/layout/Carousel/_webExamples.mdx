...
function MyCarousel() {
  ...
  function SquareAssetCard({ imageUrl, name, onPress, isVisible }) {
    return (
      <ContainedAssetCard
        description={
          ...
          <CarouselItem key={asset.symbol} id={asset.symbol} accessibilityLabel={asset.name}>
            {({ isVisible }) => (
              <SquareAssetCard
                ...
                name={asset.symbol}
                onPress={() => toast.show(`${asset.symbol} clicked`)}
              />
            )}
          </CarouselItem>
        ))}
...
If using `hideNavigation` and `autoplay`, provide an alternative pause mechanism.
...
function ComposedAutoplayCarousel() {
  ...
  function CustomPaginationDots({ totalPages, activePageIndex, onClickPage }) {
    ...
    const progressWidth = useTransform(autoplay.progress, (value) => `${value * 100}%`);
        ...
        {Array.from({ length: totalPages }, (_, index) => {
          ...
          return (
            <Pressable
              key={index}
              accessibilityLabel={`Go to page ${index + 1}`}
              background={isActive && !showProgress ? 'fgPrimary' : 'fgMuted'}
              borderRadius={1000}
              borderWidth={0}
              onClick={() => onClickPage?.(index)}
              style={{
