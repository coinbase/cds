...
DatePicker supports the same styling functionality as [DateInput](/components/other/DateInput/) for the input (`classNames.dateInput`, `styles.dateInput`) plus [Calendar](/components/other/Calendar/) slots via `classNames.calendar` and `styles.calendar`.
