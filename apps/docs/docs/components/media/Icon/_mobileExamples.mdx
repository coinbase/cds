...
- **`IconGlyphSourceProvider`** adds your glyphs to the icons CDS already renders. Use it when you want the components you're already using — `IconButton`, `Button`, `Tag` — to draw their icons from your font.
...
The `source` is the same object shape `createIcon` takes, so an icon package can export one object usable either way. Sources are consulted before the built-in CDS glyphs, which means a source that reuses a built-in name re-skins that icon rather than being ignored. Nest providers to add more than one source; the innermost one wins.
