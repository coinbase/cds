...
Loading works with all variants, transparent, and sizes. Provide `accessibilityLabel` so screen readers announce the loading state (e.g. "Loading").
...
<HStack gap={2} flexWrap="wrap" alignItems="center">
  ...
  <IconButton
    ...
    onClick={console.log}
  />
  <IconButton loading name="refresh" accessibilityLabel="Loading" size="s" onClick={console.log} />
  <IconButton loading name="refresh" accessibilityLabel="Loading" size="l" onClick={console.log} />
</HStack>
```
...
`size` accepts `xs`, `s`, `m`, and `l`. It renders at `s` when unset, because IconButton still enables the deprecated `compact` prop by default; this becomes `l` once `compact` is removed.

```jsx live
<HStack alignItems="center" flexWrap="wrap" gap={2}>
  <IconButton accessibilityLabel="Settings (xs)" name="gear" onClick={console.log} size="xs" />
  <IconButton accessibilityLabel="Settings (s)" name="gear" onClick={console.log} size="s" />
  <IconButton accessibilityLabel="Settings (m)" name="gear" onClick={console.log} size="m" />
  <IconButton accessibilityLabel="Settings (l)" name="gear" onClick={console.log} size="l" />
</HStack>
```
