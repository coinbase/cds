...
Loading works with all variants and transparent. Provide `accessibilityLabel` so screen readers announce the loading state (e.g. "Loading").
...
Use the `size` prop to control the button's dimensions. It defaults to `l`. Use `size="s"` in place of the deprecated `compact` prop (the historic implicit default was `s`).
...
<HStack gap={2} flexWrap="wrap" alignItems="center">
  ...
  <IconButton name="gear" accessibilityLabel="Large (default)" size="l" onClick={console.log} />
</HStack>
```
