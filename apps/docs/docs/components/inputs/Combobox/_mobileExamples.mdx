...
Use `accessibilityLabel` and `accessibilityHint` to describe purpose and interaction. For multi-select, add hidden-selection labels so screen readers can describe +X summaries.
...
function AccessibilityProps() {
  ...
  return (
    <Combobox
      accessibilityHint="Double tap to open options, then select one or more priorities"
