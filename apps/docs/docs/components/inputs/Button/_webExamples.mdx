...
Use `size` to choose a t-shirt size (`xs`, `s`, `m`, `l`). It defaults to `l` and replaces the deprecated `compact` prop — `size="s"` is the direct equivalent of the old `compact`.
...
```jsx live
<HStack gap={2} flexWrap="wrap">
  <Button onClick={console.log} endIcon="forwardArrow" variant="secondary" size="s">
    See more
  </Button>
  <Button onClick={console.log} endIcon="externalLink" variant="secondary" size="s">
    Open link
  </Button>
</HStack>
```
...
```jsx live
<HStack gap={2} flexWrap="wrap">
  <Button onClick={console.log} startIcon="add" startIconActive variant="secondary" size="s">
    Add item
  </Button>
  <Button onClick={console.log} startIcon="download" variant="secondary" size="s">
    Download
  </Button>
</HStack>
```
...
A full-width primary action with a secondary option.
