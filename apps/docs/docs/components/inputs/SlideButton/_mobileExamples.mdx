...
`size` accepts `s`, `m`, and `l`. Defaults to `l`. The size controls the button's height, border-radius, inner padding, and handle icon size.
  ...
  return (
    <VStack gap={2}>
      <SlideButton
        checked={checked}
        onChange={setChecked}
        uncheckedLabel="Swipe to confirm (s)"
        checkedLabel="Confirming..."
        size="s"
      />
      <SlideButton
        checked={checked}
        onChange={setChecked}
        uncheckedLabel="Swipe to confirm (m)"
        checkedLabel="Confirming..."
        size="m"
      />
      <SlideButton
        checked={checked}
        onChange={setChecked}
        uncheckedLabel="Swipe to confirm (l) — default"
        checkedLabel="Confirming..."
        size="l"
      />
    </VStack>
