...
Use the `size` prop to control the height, border-radius and inner padding of the button. Available sizes are `s`, `m` and `l`. `size` defaults to `l`. `size="s"` is the direct replacement for the deprecated `compact` prop.
