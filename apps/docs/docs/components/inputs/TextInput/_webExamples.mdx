...
If you need to render a custom label (e.g. a label with a tooltip), you can use the `labelNode` prop. This works with the default, `compact`, and `labelVariant="inside"` variants.
...
<VStack gap={2}>
  ...
  />
  <TextInput
    id="inside-label-search"
    label="Search"
    labelVariant="inside"
    labelNode={
      <InputLabel htmlFor="inside-label-search">
        <HStack alignItems="center" gap={1} paddingTop={1}>
          Search
          <Tooltip content="Search across all assets.">
            <Icon active color="fg" name="info" size="xs" tabIndex={0} />
          </Tooltip>
        </HStack>
      </InputLabel>
