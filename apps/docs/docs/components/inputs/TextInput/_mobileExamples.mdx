...
<VStack gap={3}>
  ...
  <VStack>
    <Text as="p">Use a smaller size when space is tight.</Text>
    <TextInput
      size="s"
      type="number"
      step="0.01"
...
<VStack gap={3}>
  ...
  <VStack>
    <Text as="p">Right aligned Used with size s</Text>
    <TextInput
      label="Limit price"
      size="s"
      align="end"
...
Controls vertical density of the input. Defaults to `l` (16px paddingY). Use `size="s"` instead of the deprecated `compact` prop when you want a denser field that still allows an outside or inside label.

```jsx
<VStack gap={2}>
  <TextInput label="Small" placeholder="size s" size="s" />
  <TextInput label="Medium" placeholder="size m" size="m" />
  <TextInput label="Large (default)" placeholder="size l" size="l" />
</VStack>
```

### Label Variants

TextInput supports two label variants: `outside` (default) and `inside`. For `size="s"` or `size="m"`, an inside label renders horizontally in the start slot; for `size="l"`, it stacks vertically inside the field. The deprecated `compact` prop forces the label into the start slot regardless of `labelVariant`.
...
<VStack gap={3}>
  ...
  <TextInput label="Label" size="s" disabled />
</VStack>
```
...
function TextInputKeyboardExample() {
  return (
    <ScrollView style={{ height: '100%' }} keyboardShouldPersistTaps="always">
      <TextInput label="Amount" type="number" size="s" placeholder="98329.23" suffix="USD" />
    </ScrollView>
  );
}
```
