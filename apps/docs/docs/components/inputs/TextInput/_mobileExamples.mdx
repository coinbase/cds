...
```jsx
<TextInput label="Email" placeholder="you@example.com" size="s" />
```
...
TextInput supports two label variants: `outside` (default) and `inside`. `size` and `labelVariant` are independent — use `labelVariant="inside"` with any size. At sizes `s` and `m`, an inside label sits inline in the start slot; at size `l`, it stacks vertically above the field.
...
<VStack gap={3}>
  ...
  <TextInput label="Label" size="s" labelVariant="inside" disabled />
</VStack>
```
