...
Control whether the select has a visible border. The `bordered` prop defaults to `true`.

```jsx live
function BorderedExample() {
  const [borderedValue, setBorderedValue] = useState('1');
  const [borderlessValue, setBorderlessValue] = useState('1');
