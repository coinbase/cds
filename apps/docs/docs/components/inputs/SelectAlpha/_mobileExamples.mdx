...
Use the `size` prop to control the density of the Select input. `size` defaults to `l`, and
`size="s"` replaces the deprecated `compact` prop.
...
function SizeExample() {
  ...
  return (
    <VStack gap={3}>
      <Select
        size="s"
        label="Small (size=s, replaces compact)"
        value={value}
        onChange={setValue}
        options={options}
        placeholder="Select an option"
...
The `size` prop works with multi-selection mode as well.

```jsx
function MultiSelectSizeExample() {
  const { value, onChange } = useMultiSelect({
    initialValue: ['1', '3'],
  });
...
Combine the small size with different variants.
  ...
  return (
    <VStack gap={4}>
      <Select
        ...
        onChange={setValue1}
        options={options}
        helperText="Success state in small size"
      />
      ...
      <Select
        ...
        onChange={setValue2}
        options={options}
        helperText="Error state in small size"
      />
    </VStack>
  );
}
