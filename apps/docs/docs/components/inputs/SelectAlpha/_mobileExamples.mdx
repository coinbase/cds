...
Multi-selection mode works at every size. When `size="s"`, the dropdown uses compact row density to match the smaller control.

```jsx
function SizesMultiSelectExample() {
  const { value, onChange } = useMultiSelect({
    initialValue: ['1', '3'],
  });
...
Combine `size="s"` with different variants.
