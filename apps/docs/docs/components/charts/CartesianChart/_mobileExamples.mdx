...
CartesianChart supports data highlighting when `enableHighlighting` is true. Use `onHighlightChange` to react as the user interacts with the chart.

One example of using the scrubber is to provide haptic feedback when the user interacts with the chart. You can trigger a light impact each time the highlighted position changes or even do a dynamic impact depending on the value change, such as a heavy impact when the user crosses a significant boundary of time or reaches a significant market event.
...
function Highlighting() {
  ...
  return (
    <VStack gap={2}>
      <Text font="label1">Highlighted index: {scrubIndex ?? 'none'}</Text>
      <LineChart
        enableHighlighting
...
function FiatAndStablecoinBalance() {
  ...
  return (
    <VStack gap={2} width="100%">
      <VStack gap={2} width="100%">
        <VStack paddingY={1} paddingX={2} bordered borderRadius={400} flexGrow={1} gap={2}>
          <VStack>
            <Text font="headline" color="fgMuted">
              Deposits
            </Text>
            <Text font="title2">{depositsSummary}</Text>
          </VStack>
          <BarChart
            showXAxis
            stacked
        ...
        <VStack paddingY={1} paddingX={2} bordered borderRadius={400} flexGrow={1} gap={2}>
          <VStack>
            ...
            <Text font="title2">{balanceSummary}</Text>
          </VStack>
          <AreaChart
            styles={{ chart: { overflow: 'visible' } }}
            showXAxis
            showYAxis
            stacked
            enableHighlighting
            ...
            yAxis={{
              showGrid: true,
              domain: { min: 0 },
              tickLabelFormatter: formatValueAxis,
            }}
          >
            <Scrubber />
          </AreaChart>
      ...
      <HStack
        ...
        width="100%"
      >
        <DefaultLegendEntry color="#009B3A" label="BRL" shape="circle" />
        <DefaultLegendEntry color="#2775CA" label="USDC" shape="circle" />
        <DefaultLegendEntry color="#5b6cff" label="USD" shape="circle" />
      </HStack>
    </VStack>
  );
}
...
<BarChart
  ...
  height={220}
  series={[
    { id: 'series1', data: [5, 1, 3], color: 'var(--color-accentBoldBlue)', label: 'Series 1' },
    { id: 'series2', data: [2, 4, 6], color: 'var(--color-accentBoldYellow)', label: 'Series 2' },
    { id: 'series3', data: [1, 2, 3], color: 'var(--color-accentBoldRed)', label: 'Series 3' },
  ]}
  legend
  showXAxis
  showYAxis
