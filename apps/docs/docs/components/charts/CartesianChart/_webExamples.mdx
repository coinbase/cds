...
Gradient configuration is shared across [AreaChart](/components/charts/AreaChart/), [LineChart](/components/charts/LineChart/), and [BarChart](/components/charts/BarChart/) through the `series[].gradient` prop.
...
You can also pass separate gradients to `Line` and `Area` layers inside `CartesianChart`.

```jsx live
function LayerGradients() {
...
Todo show an example where we have a chart which is automatically resized

#### Sparkline

Todo showcase some example sparkline charts (bar, area, and line)
...
[AreaChart](/components/charts/AreaChart/), [LineChart](/components/charts/LineChart/), and [BarChart](/components/charts/BarChart/) expose this same animation model via their `transitions` and `animate` props.

```jsx live
function WrapperChartAnimations() {
  const data = [10, 22, 29, 45, 98, 45, 22];
...
You can also target specific children directly, such as animating both the line path and the scrubber indicator.

```jsx live
function ChildTransitions() {
  const data = [10, 22, 29, 45, 98, 45, 22];
  ...
  return (
    ...
    >
      ...
      />
      <Scrubber transitions={{ update: { type: 'spring', stiffness: 700, damping: 20 } }} />
    </CartesianChart>
  );
}
```
...
Bar transitions support `staggerDelay` (cascade timing across bars) and `delay` (fixed wait before animation starts).
...
<BarChart
  ...
  transitions={{
    enter: { type: 'spring', stiffness: 700, damping: 80, staggerDelay: 250, delay: 100 },
  }}
/>
```
...
Show an example where we have a table of assets, with their amount, USD value, and chart of recent performance
