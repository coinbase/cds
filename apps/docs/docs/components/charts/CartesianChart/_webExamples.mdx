...
Build a streaming candlestick chart with custom SVG children and framer-motion transitions. Set `animate={false}` on `CartesianChart` so child components own enter, exit, shift, and price-update animations. Custom `GridLineComponent` and `TickLabelComponent` keep axis labels and grid lines in sync with the y-domain.

```jsx live
function LiveCandlesticks() {
  const liveCandleCount = 30;
  const liveTickIntervalMinMs = 250;
  const liveTickIntervalMaxMs = 1000;
  const liveLoopDurationMs = 60000;
  const liveCandleDurationMs = 6000;
  const liveTicksPerCandle = 12;
  const liveBasePrice = 50000;
  const liveTickStepMin = 75;
  const liveTickStepMax = 450;
  const livePriceTickStep = 2000;
  const livePriceTickMin = 40000;
  const livePriceTickMax = 60000;
  ...
  const candlePriceTransition = { type: 'spring', stiffness: 1400, damping: 90, mass: 4 };
  const candleShiftTransition = { type: 'spring', stiffness: 1200, damping: 85, mass: 4 };
  const candleExitTransition = { type: 'tween', duration: 0.25 };
  ...
  const parseHorizontalLinePath = (path) => {
    if (!path) return null;
  ...
  const tickLiveCandle = (candle, tickIndex) => {
  ...
  const LiveAxisGridLine = memo(({ d, stroke, strokeWidth = 1 }) => {
  ...
  const AnimatedCandle = memo(
    ({ candle, slotIndex, isExiting = false, isEntering = false, onExitComplete }) => {
  ...
  const AnimatedCandlestickPlot = memo(
    ({ candles, enteringCandleId, exitingCandles, onExitComplete }) => {
      const { drawingArea, getXScale, getYScale } = useCartesianChartContext();
      const xScale = getXScale();
      const yScale = getYScale();
  ...
  function Chart() {
    ...
    const createInitialLiveCandles = () => {
      ...
      for (let index = 0; index < liveCandleCount; index++) {
        const result = buildLiveCandle(price, tickIndex, createLiveCandle);
    ...
    const shiftLiveCandles = (candles) => {
    ...
    const initialLiveState = useMemo(() => createInitialLiveCandles(), []);
    const [candles, setCandles] = useState(() => initialLiveState.candles);
    const [exitingCandles, setExitingCandles] = useState([]);
    const [enteringCandleId, setEnteringCandleId] = useState(null);
    const tickIndexRef = useRef(initialLiveState.tickIndex);
    const loopStartRef = useRef(Date.now());
    const lastShiftRef = useRef(Date.now());

    const handleExitComplete = useCallback((candleId) => {
        ...
        timeoutId = setTimeout(() => {
          ...
          if (now - lastShiftRef.current >= liveCandleDurationMs) {
            ...
            setCandles((current) => {
              ...
              setExitingCandles((exiting) => [
                ...exiting,
                { candle: shifted.exited, slotIndex: 0 },
              ]);
