...
BarChart supports screen reader accessibility through `enableScrubbing` and `scrubberAccessibilityLabel`. You do not need to add a [Scrubber](/components/charts/Scrubber) component—the chart renders invisible tap targets that screen reader users can navigate with swipe or tap.
...
function AccessibleBarChart() {
  ...
  return (
    <BarChart
      ...
      showXAxis
      showYAxis
      accessibilityLabel={`Bar chart with ${values.length} months. Tap segments to navigate.`}
      height={180}
