...
Charts support interaction when `enableHighlighting` is true. Use `onHighlightChange` to listen for updates; it supports multiple highlighted items. Adding `Scrubber` to LineChart shows the visual scrubber for the current position.
...
const MOBILE_FORECAST_EXAMPLE_STROKE_WIDTH = 3;
const MOBILE_FORECAST_EXAMPLE_CLIP_OFFSET = MOBILE_FORECAST_EXAMPLE_STROKE_WIDTH;
const MOBILE_FORECAST_EXAMPLE_SPLIT_INDEX = 6;
const MOBILE_FORECAST_EXAMPLE_CHART_DATA: number[] = [50, 45, 47, 46, 54, 54, 60, 61, 63, 66, 70];
...
const MobileForecastAssetPriceHistoricalLine = memo((props: SolidLineProps) => {
  ...
  return (
    <Group clip={historicalClipPath}>
      <SolidLine strokeWidth={MOBILE_FORECAST_EXAMPLE_STROKE_WIDTH} {...props} />
    </Group>
  );
});
...
const MobileForecastAssetPriceFutureLine = memo((props: DottedLineProps) => {
  ...
  return (
    <Group clip={forecastClipPath}>
      <DottedLine
        dashIntervals={[0, MOBILE_FORECAST_EXAMPLE_STROKE_WIDTH * 2]}
        strokeWidth={MOBILE_FORECAST_EXAMPLE_STROKE_WIDTH}
        {...props}
      />
    </Group>
  );
...
function ForecastAssetPrice() {
  ...
  const [userHighlight, setUserHighlight] = useState<HighlightedItem[]>([]);
  ...
  return (
    ...
    >
      <Line
        LineComponent={MobileForecastAssetPriceHistoricalLine}
        curve="linear"
        seriesId="price"
      />
      <Line
        LineComponent={MobileForecastAssetPriceFutureLine}
        curve="monotone"
        seriesId="price"
        type="dotted"
      />
      <XAxis position="bottom" requestedTickCount={3} tickLabelFormatter={axisFormatter} />
      <MobileForecastScrubberWrap hideLine={hideScrubberLine} />
    </CartesianChart>
  );
}
```
