...
<LineChart
  ...
  accessibilityLabel="Chart with overflow gestures. Tap segments to navigate."
  scrubberAccessibilityLabel={(index) => `Point ${index + 1}`}
  ...
>
  ...
</LineChart>
...
`scrubberAccessibilityLabel` defines the per-segment text announced by screen readers. You do not need to add a `Scrubber` component for accessibility—`enableScrubbing` with `scrubberAccessibilityLabel` is sufficient. BarChart and other charts work the same way.
...
function BasicAccessible() {
  ...
  const chartAccessibilityLabel = useMemo(
    () =>
      `Price chart showing trend over ${data.length} data points. Current value: ${data[data.length - 1]}. Tap segments to navigate.`,
    [data],
  );
...
function AssetPriceWithDottedArea() {
  ...
  const AssetPriceDotted = memo(() => {
    ...
    const chartAccessibilityLabel = useMemo(
      () =>
        `Bitcoin price chart for ${timePeriod.label} period. Current price: ${formatPrice(currentPrice)}. Tap segments to navigate.`,
