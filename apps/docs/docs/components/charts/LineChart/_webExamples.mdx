...
Charts support interaction when `enableHighlighting` is true. Use `onHighlightChange` to listen for updates. Adding `Scrubber` to LineChart shows the visual scrubber for the current position.
...
You can combine multiple lines within a series to change styles dynamically. You can also control highlight state to keep the scrubber on the forecast boundary at rest and follows the pointer while scrubbing.

```jsx live
// Stable LineComponent types + clip constants: defining these inside `ForecastAssetPrice` recreates
// them every `userHighlight` update and invalidates `Line`’s memo and path work.
const FORECAST_EXAMPLE_STROKE_WIDTH = 3;
const FORECAST_EXAMPLE_CLIP_OFFSET = FORECAST_EXAMPLE_STROKE_WIDTH;
const FORECAST_EXAMPLE_SPLIT_INDEX = 6;

const ForecastAssetPriceHistoricalLine = memo((props) => {
  ...
  return (
    <>
      <defs>
        <clipPath id="doc-forecast-asset-historical-clip">
          <rect
            height={drawingArea.height + FORECAST_EXAMPLE_CLIP_OFFSET * 2}
            width={currentX + FORECAST_EXAMPLE_CLIP_OFFSET - drawingArea.x}
            x={drawingArea.x - FORECAST_EXAMPLE_CLIP_OFFSET}
            y={drawingArea.y - FORECAST_EXAMPLE_CLIP_OFFSET}
          />
        </clipPath>
      </defs>
      <g clipPath="url(#doc-forecast-asset-historical-clip)">
        <SolidLine strokeWidth={FORECAST_EXAMPLE_STROKE_WIDTH} {...props} />
      </g>
    </>
  );
});
...
const ForecastAssetPriceFutureLine = memo((props) => {
  ...
  return (
    <>
      <defs>
        <clipPath id="doc-forecast-asset-future-clip">
          <rect
            height={drawingArea.height + FORECAST_EXAMPLE_CLIP_OFFSET * 2}
            width={drawingArea.x + drawingArea.width - currentX + FORECAST_EXAMPLE_CLIP_OFFSET * 2}
            x={currentX}
            y={drawingArea.y - FORECAST_EXAMPLE_CLIP_OFFSET}
          />
        </clipPath>
      </defs>
      <g clipPath="url(#doc-forecast-asset-future-clip)">
        <DottedLine
          strokeDasharray={`0 ${FORECAST_EXAMPLE_STROKE_WIDTH * 2}`}
          strokeWidth={FORECAST_EXAMPLE_STROKE_WIDTH}
          {...props}
        />
      </g>
    </>
...
const ForecastAssetPriceScrubber = memo(function ForecastAssetPriceScrubber({ hideLine }) {
  return (
    <m.g animate={{ opacity: 1 }} initial={{ opacity: 0 }} transition={FORECAST_EXAMPLE_MOTION}>
      <Scrubber hideLine={hideLine} hideOverlay />
    </m.g>
  );
});
...
function ForecastAssetPrice() {
  ...
  return (
    <CartesianChart
      ...
      height={FORECAST_EXAMPLE_CHART_HEIGHT}
      maxWidth={512}
      series={series}
      style={FORECAST_EXAMPLE_CHART_STYLE}
    >
      <Line LineComponent={ForecastAssetPriceHistoricalLine} curve="linear" seriesId="price" />
      <Line
        LineComponent={ForecastAssetPriceFutureLine}
        curve="monotone"
        seriesId="price"
        type="dotted"
      />
      <XAxis position="bottom" requestedTickCount={3} tickLabelFormatter={axisFormatter} />
      <ForecastAssetPriceScrubber hideLine={hideScrubberLine} />
    </CartesianChart>
  );
}
```
