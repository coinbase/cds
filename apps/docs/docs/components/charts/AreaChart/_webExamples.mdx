AreaChart is a wrapper for [CartesianChart](/components/charts/CartesianChart) that has some unique features over [LineChart](/components/charts/LineChart), such as the ability to stack areas on top of each other and a default minimum domain of 0. Charts are built using SVGs.
...
You can compare different curve interpolation strategies on a per-series basis.
...
Legend entries are driven by each series' `label`, `color`, and `legendShape`.
...
<AreaChart
  ...
  showLines
  showYAxis
  height={{ base: 190, tablet: 230, desktop: 270 }}
  series={[
    {
      id: 'retained',
      label: 'Retained',
      data: [62, 64, 63, 66, 68, 70],
      color: 'var(--color-fgPositive)',
      legendShape: 'pill',
    },
    {
...
`showLines` can be combined with chart-level and series-level line styling.
...
<AreaChart
  ...
  series={[
    ...
    },
    {
      id: 'forecast',
      label: 'Forecast',
      data: [46, 50, 54, 58, 64, 72, 82],
      color: 'var(--color-fgPositive)',
      type: 'dotted',
      LineComponent: (props) => <DottedLine {...props} strokeDasharray="2 8" />,
      legendShape: 'pill',
    },
  ]}
  yAxis={{
...
You can pair tuple data with a projected line to communicate uncertainty and expected direction together.
