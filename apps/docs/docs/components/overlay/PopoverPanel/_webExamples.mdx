...
Always pass a natively interactive element — such as [`Button`](/components/buttons/Button/), [`IconButton`](/components/buttons/IconButton/), or an `<a>` tag — as `children`. Non-interactive elements (plain `div`, `span`, `Text`) are not reachable by keyboard and are invisible to assistive technology, which breaks accessibility.
:::
