...
When providing a custom TabComponent, use `useTabsContext()` and call `updateActiveTab(id)` to update selection state. Reflect the active state (e.g., `inverted` styling) based on `activeTab?.id === id`. Mirror selection with `accessibilityState={{ selected: isActive }}`.
:::
