...
Use `size` to control the height and spacing of each tab chip. `size` defaults to `s`; use
`size="xs"` for dense layouts (this replaces the deprecated `compact` prop).

```tsx
function ExampleSizeNoStart() {
  const tabs = [
    { id: 'all', label: 'All' },
    { id: 'swap', label: 'Swap' },
    { id: 'collect', label: 'Collect' },
  ...
  const [activeTab, setActiveTab] = useState(tabs[0]);
  return <TabbedChips activeTab={activeTab} onChange={setActiveTab} size="xs" tabs={tabs} />;
}
```
...
function ExampleWithStart() {
  ...
  return (
    <VStack gap={2}>
      <TabbedChips activeTab={activeTab} onChange={setActiveTab} tabs={tabs} />
      <TabbedChips activeTab={activeTab} onChange={setActiveTab} size="xs" tabs={compactTabs} />
    </VStack>
  );
}
```
