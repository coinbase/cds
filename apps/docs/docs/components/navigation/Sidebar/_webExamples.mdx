...
function RenderEndExample() {
  ...
  return (
    <HStack alignItems="flex-start" justifyContent="center" overflow="hidden">
      <Sidebar logo={<LogoMark />} renderEnd={renderEnd}>
        {items.map((item, index) => (
          <SidebarItem
            key={item.title}
            active={index === activeIndex}
...
Use the `styles` and `classNames` props to customize specific parts of the sidebar. Both props accept objects with keys for `root`, `logo`, `content`, and `end`.
...
function TradingInterface() {
  ...
  return (
    <HStack alignItems="stretch" height={350} overflow="hidden">
      <Sidebar logo={<LogoMark foreground />} variant="condensed">
        {items.map((item, index) => (
          <SidebarItem
            key={item.title}
