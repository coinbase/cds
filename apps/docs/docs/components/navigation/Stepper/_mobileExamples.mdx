...
In this example, the "Payment Method" step displays a `circleCross` icon, negative-colored label, and red progress line once the step has been visited.

```jsx
const ErrorStepperIcon = memo(function ErrorStepperIcon({
  depth,
  active,
  visited,
  complete,
...
const ErrorStepperLabel = memo(function ErrorStepperLabel({
  step,
  depth,
  active,
  visited,
