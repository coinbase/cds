...
Set `borderRadius` on SegmentedTabs to change the shape of both the container and the active indicator automatically.
...
Use the `styles` prop to customize individual elements of the SegmentedTabs. This example sets a different `borderRadius` on the active indicator than the outer container, with padding to create an inset effect.
