  ...
  function Chart() {
    ...
    return (
      <CartesianChart
        ...
        inset={{ top: 16, bottom: 16, left: 16, right: 16 }}
        series={[
          { id: 'line', data, color: theme.color.accentBoldBlue },
          { id: 'area', data: data.map((d) => d * 0.6), color: theme.color.accentBoldGreen },
          { id: 'bars', data: data.map((d) => d * 0.3), color: theme.color.accentBoldPurple, yAxisId: 'bars' },
        ]}
