...
CartesianChart delegates transition control to its child components. Each `Line`, `Area`, and `Bar` (or the convenience wrappers `LineChart`, `AreaChart`, `BarChart`) accepts a `transitions` prop with `enter` (clip-path reveal) and `update` (data-change morph) keys. Set either to `null` to disable that phase. You can also disable all animations chart-wide by passing `animate={false}` on CartesianChart.
...
Each child component can define its own transitions independently. Here, the `Line` uses a spring morph while the `Area` uses a slow tween, and the bars snap with no update animation. This lets you fine-tune each visual layer within a single chart.
  ...
  function Chart() {
    ...
    return (
      <CartesianChart
        ...
        inset={{ top: 16, bottom: 16, left: 16, right: 16 }}
        series={[
          { id: 'line', data, color: 'var(--color-accentBoldBlue)' },
          { id: 'area', data: data.map((d) => d * 0.6), color: 'var(--color-accentBoldGreen)' },
          { id: 'bars', data: data.map((d) => d * 0.3), color: 'var(--color-accentBoldPurple)', yAxisId: 'bars' },
        ]}
      ...
      >
        ...
        <Area
          seriesId="area"
          transitions={{
            update: { type: 'tween', duration: 1.5 },
          }}
        />
        <Line
          seriesId="line"
...
You can disable all animations chart-wide by setting `animate` to `false` on CartesianChart. This is useful for static snapshots or when performance is a concern.
