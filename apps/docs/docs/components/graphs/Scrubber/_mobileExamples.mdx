...
You can use `BeaconLabelComponent` to display a label with the percentage value of the data at the scrubber position. This example uses `SkParagraph` to style the percentage in bold and the label in regular weight.
