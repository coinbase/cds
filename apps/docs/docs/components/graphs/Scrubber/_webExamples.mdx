...
You can use `BeaconComponent` to customize the visual appearance of scrubber beacons. This simplest way is to modify the props for `DefaultScrubberBeacon`.
...
<Box borderRadius={300} padding={2} style={{ background: 'rgb(var(--red40))' }}>
  ...
  >
    <Scrubber
      ...
      BeaconComponent={(props) => <DefaultScrubberBeacon {...props} stroke="rgb(var(--red40))" />}
      hideOverlay
    />
  </LineChart>
</Box>
...
You can also pass a custom component to `BeaconComponent` to further customize the visual appearance of scrubber beacons.
