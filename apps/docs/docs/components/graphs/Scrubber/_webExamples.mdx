...
setting `label` on Scrubber displays a label above the scrubber line (only when scrubbing)
...
Using `elevateLabel` will elevate the Scrubber's reference line label with a shadow.
