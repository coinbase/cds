...
You can customize the transition animations for your bar chart using the `transitions` prop.

```jsx
function UpdatingChartValues() {
  const [data, setData] = useState([45, 80, 120, 95, 150, 110, 85]);
  const [nullIndex, setNullIndex] = useState(null);
  ...
  return (
    <VStack gap={2}>
      <HStack gap={2}>
        ...
        <Button compact onPress={() => setNullIndex((prev) => (prev === null ? 3 : null))}>
