...
You can add points using `renderPoints` on Line or [LineChart](/components/graphs/LineChart).
...
You can use `labelPosition` and `labelOffset` to adjust Point's label.
