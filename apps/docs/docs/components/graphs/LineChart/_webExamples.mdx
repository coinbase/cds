...
You can use the `gradient` prop on `series` or `Line` components to enable gradients. Gradients can be applied to the y-axis (default) or x-axis. Each stop requires an `offset`, which is based on the data within the x/y scale and `color`, with an optional `opacity` (defaults to 1).
...
function ForecastAssetPrice() {
  ...
  const CustomScrubber = memo(() => {
    ...
    if (scrubberPosition === undefined) {
      return <ScrubberBeacon seriesId="price" dataX={currentIndex} dataY={data[currentIndex]} />;
    }
    return <Scrubber hideOverlay />;
  });

  // We are disabling animations since the scrubber change has a delay
  return (
    <CartesianChart
      animate={false}
      enableScrubbing
