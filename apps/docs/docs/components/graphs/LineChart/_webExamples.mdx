...
function GainLossChart() {
  ...
  return (
    <Chart
      ...
      padding={{ top: 12, bottom: 12, left: 16, right: 0 }}
...
const BTCActiveIndicator = memo(({ style, ...props }: TabsActiveIndicatorProps) => (
  ...
  return (
    <VStack gap={2}>
      ...
      >
        <Scrubber scrubberLabel={scrubberLabel} scrubberLabelProps={{ elevation: 1 }} pulse />
      </LineChart>
      <PeriodSelector
        TabComponent={BTCTab}
    ...
    >
      ...
      >
        <LineChart
          ...
          height={92}
        >
          <Scrubber pulse styles={{ scrubberHead: { stroke: 'white' } }} />
        </LineChart>
      </div>
    </VStack>
  );
