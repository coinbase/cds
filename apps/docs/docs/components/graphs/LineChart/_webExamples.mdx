...
You can use `renderPoints` from LineChart with `onClick` listeners to render instances of [Point](/components/graphs/Point) that are interactable.
...
[todo talk about domain and range]
...
>
  <ReferenceLine
    dataY={10}
    stroke="var(--color-fg)"
    LineComponent={(props) => <DottedLine {...props} strokeWidth={3} strokeDasharray="0 16" />}
  />
  <Scrubber />
</LineChart>
```
...
function ResizableLineChart() {
  ...
  function DetailCell ({ title, description }) {
    return (
      <VStack>
        <Text font="label2" color="fgMuted">{title}</Text>
  ...
  return (
    <HStack gap={3}>
      ...
      <VStack gap={1}>
        ...
        <DetailCell title="High" description={formatPrice(periodHigh)} />
        <DetailCell title="Low" description={formatPrice(periodLow)} />
        <VStack gap={1} display={{ base: 'none', tablet: 'flex', desktop: 'flex' }}>
          <DetailCell title="24h" description={formatPercentage(change24h)} />
          <DetailCell title="7d MA" description={formatPrice(latestMA7)} />
        </VStack>
      </VStack>
    </HStack>
  );
