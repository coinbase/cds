...
By default, charts will use the default font of the system. You can provide a font family at the chart level to customize this. For more, see [Skia's documentation on fonts](https://shopify.github.io/react-native-skia/docs/text/paragraph/#fonts).
...
function MonotoneAssetPrice() {
  ...
  const scrubberLabel = useCallback(
    ...
    [scrubberPriceFormatter, prices, formatDate, theme.color.fgMuted],
  );
  ...
  const CustomScrubberBeacon = memo(({ dataX, dataY, seriesId, isIdle, animate = true }: ScrubberBeaconProps) => {
    ...
    return (
      <>
        ...
        <Circle c={animatedPoint} color={theme.color.fgPrimary} r={5} style="stroke" strokeWidth={3} />
      </>
    );
  });
      ...
      series={[
        {
          ...
          gradient: {
            axis: 'x',
            stops: ({ min, max }) => [
              { offset: min, color: theme.color.fgPrimary, opacity: 0 },
              { offset: 32, color: theme.color.fgPrimary, opacity: 1 },
            ],
          },
        },
      ]}
