...
You can configure chart transitions using `transitionConfig` on LineChart and [Scrubber](/components/graphs/Scrubber). You can also disable animations by setting the `animate` on LineChart to `false`.
...
function GainLossChart() {
  ...
  const negativeColor = `rgb(${theme.spectrum.gray50})`;
  const positiveColor = theme.color.fgPositive;
