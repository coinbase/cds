...
You can configure chart transitions using `transition` on LineChart and `beaconTransitions` on [Scrubber](/components/graphs/Scrubber). Use `enter` for the clip enter animation and `update` for path morphing. Set either to `null` to disable that animation.

:::note Deprecation
Passing a single Transition object to `transition` or using the `animate` prop is deprecated. Prefer `transition={{ enter, update }}` and set `enter` or `update` to `null` to disable.
:::
    ...
    return (
      ...
      >
        ...
        <Line
          ...
          strokeWidth={3}
          transition={{
            enter: { type: 'timing', duration: 600 },
