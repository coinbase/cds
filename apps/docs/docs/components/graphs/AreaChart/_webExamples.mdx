...
You can configure chart transitions using `transition={{ enter, update }}`. Use `enter` for the clip enter animation and `update` for path morphing. Set either to `null` to disable that animation.

:::note Deprecation
Passing a single Transition object to `transition` or using the `animate` prop is deprecated. Prefer `transition={{ enter, update }}`.
:::

### Customized Transitions

You can pass separate enter and update transitions to control how the area reveals and morphs over time.
...
function AnimatedStackedAreas() {
  ...
  const enterTransition = { duration: 0.35 };
  const updateTransition = { type: 'spring', stiffness: 700, damping: 20 };
  const transitions = { enter: enterTransition, update: updateTransition };
...
You can disable animations by setting `transition={{ enter: null, update: null }}`. The `animate={false}` prop is deprecated but still supported.
...
function AnimatedStackedAreas() {
  ...
  function AnimatedChart() {
    ...
    return (
      <AreaChart
        transition={{ enter: null, update: null }}
        overflow="visible"
        stacked
        height={{ base: 200, tablet: 250, desktop: 300 }}
        series={series}
