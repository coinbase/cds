...
On band scales, you can also use `bandGridPosition` to control where grid lines appear relative to each band.
...
function BandGridPosition() {
  const [selectedBandGridPosition, setSelectedBandGridPosition] = useState('edges');
  ...
  return (
    ...
    >
      <XAxis
        bandGridPosition={selectedBandGridPosition}
...
function BandTickMarkPosition() {
  const [selectedBandTickMarkPosition, setSelectedBandTickMarkPosition] = useState('middle');
  ...
  return (
    ...
    >
      <XAxis
        bandTickMarkPosition={selectedBandTickMarkPosition}
        showLine
        showTickMarks
        tickMarkSize={8}
      />
