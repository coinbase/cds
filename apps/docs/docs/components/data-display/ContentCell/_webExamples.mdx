:::tip
Prefer using `ListCell` for new experiences. `ContentCell` remains available for legacy surfaces that cannot yet migrate to `ListCell`, but new work should start with `ListCell`. See [ContentCell with ListCell](../ListCell/#contentcell) for a full example.
:::
