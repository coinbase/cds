...
The `title`, `description`, `detail`, and `subdetail` string props are rendered inside a CDS `Text` with default fonts and truncation. To render arbitrary React nodes (icons, multiple elements, chips, etc.), use the Node props: `titleNode`, `descriptionNode`, `detailNode`, and `subdetailNode`.
...
<ListCell
  ...
  accessoryNode={
    <IconButton accessibilityLabel="Help" name="info" onPress={() => alert('Accessory pressed')} />
  }
/>
```
