...
:::note
Prefer `spacingVariant="condensed"` for the new ListCell design. The `normal` and `compact` values exist for backward compatibility and may be removed in a future major release.
:::
