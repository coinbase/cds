...
The `priority` prop controls which parts of the cell are protected from shrinking and truncation when horizontal space is limited. Use it to make a specific region “win” space before others when clipping occurs.

- **start**: the main text column (`title`/`description`)
- **middle**: the `intermediary` region
- **end**: the trailing region (either `end` content or `detail`/`subdetail`)

You can pass a single value (e.g., `"start"`) or an array (e.g., `['start', 'end']`) to protect multiple regions. The example below constrains width to 240px to make truncation behavior clear.

```tsx live
function PriorityCOntent() {
  const dimensions = { width: 62, height: 18 };
  const sparklineData = Array.from({ length: 24 }, (_, i) => 100 + Math.sin(i / 2) * 8 + (i % 3));
