  ...
  beforeEach(() => {
    ...
    fs.readFileSync = (filePath, encoding) => {
      ...
      if (normalizedPath.includes('/packages/web/package.json')) {
        return JSON.stringify({ name: '@cbhq/cds-web' });
      }
      if (normalizedPath.includes('/packages/mobile/package.json')) {
        return JSON.stringify({ name: '@cbhq/cds-mobile' });
      }
      if (normalizedPath.includes('/packages/web-visualization/package.json')) {
        return JSON.stringify({ name: '@cbhq/cds-web-visualization' });
      }
      if (normalizedPath.includes('/packages/mobile-visualization/package.json')) {
        return JSON.stringify({ name: '@cbhq/cds-mobile-visualization' });
      }
    ...
    valid: [
      {
        ...
        code: `
          ...
          figma.connect(Button, 'https://figma.com/design/abc', {
            imports: ["import { Button } from '@cbhq/cds-web/buttons/Button';"],
            props: {},
            example: () => <Button />,
          });
        `,
        ...
        errors: [
          {
            ...
            data: {
              ...
              expectedPackage: '@cbhq/cds-mobile',
            },
          },
        ],
      },
