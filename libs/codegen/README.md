...
Scans all mobile story files (`packages/**/mobile/src/**/__stories__/*.stories.tsx`) and generates the route table for `apps/mobile-app/src/routes.ts`.
...
Reads every SVG file from `packages/icons/src/svgs/` and generates a static map at `apps/mobile-app/src/__generated__/iconSvgMap.ts`. The map keys are `iconName-size-state` (e.g. `account-24-active`) and each value holds the raw SVG string, used by mobile-app to render icons directly via `react-native-svg`.
