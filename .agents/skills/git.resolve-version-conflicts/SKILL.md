...
> **Prefer re-running the release when the plan is recoverable.** Versions and
> changelog entries are generated by `nx release` from the version plans in
> `.nx/version-plans/`. If you still have the branch's plan file (or can recreate it
> with `yarn nx release plan`), the cleanest fix is to take `master`'s side of both
> conflicted files, restore the plan, and re-run `yarn release`. Use the manual
> recomputation below only when that is not practical. See `docs/release.md`.
