...
describe('Button', () => {
  ...
  describe('size', () => {
    ...
    it('resolves each t-shirt size to the expected geometry and font', () => {
      const cases = [
    ...
    it('lets `size` win when both `size` and `compact` are provided', () => {
      ...
      expect(screen.getByRole('button').className).toMatch(/(^|\s)_900-/);
    });

    it('lets an explicit `font` override the size-derived default', () => {
      render(
        <DefaultThemeProvider>
          <Button font="body" size="xs">
            Child
          </Button>
        </DefaultThemeProvider>,
      );
