...
export type SkiaCartesianChartBaseProps = BoxBaseProps &
...
export type SkiaCartesianChartProps = SkiaCartesianChartBaseProps &
...
/**
 * SkiaCartesianChart - A Skia-powered version of CartesianChart
 *
 * This component uses @shopify/react-native-skia instead of react-native-svg for rendering.
 * It provides the same API as CartesianChart but with better performance characteristics.
 *
 * Migration notes from SVG to Skia:
 * - Replace <Svg> with <Canvas>
 * - Replace SVG elements with Skia equivalents:
 *   - <Path> → <Path> (but with Skia API)
 *   - <Rect> → <Rect> (but with Skia API)
 *   - <Circle> → <Circle> (but with Skia API)
 *   - <G> → <Group>
 *   - <Defs> → Not needed, use direct references
 *   - <LinearGradient> → useShader() hooks
 * - Skia uses a declarative API but with different prop names:
 *   - fill="color" → color="color"
 *   - stroke="color" → strokeWidth={width} style="stroke"
 *   - strokeWidth → strokeWidth (same but required when style="stroke")
 * - Animations work differently:
 *   - Use useValue() for animated values
 *   - Use useComputedValue() for derived animations
 *   - Use withTiming() from Skia (not reanimated's withTiming)
 */
export const SkiaCartesianChart = memo(
  forwardRef<View, SkiaCartesianChartProps>(
    (
      {
        series,
        animate = true,
        ...
        <CartesianChartProvider value={contextValue}>
          ...
          >
            <Box
              ...
              {...props}
            >
              <Canvas style={{ width: chartWidth, height: chartHeight }}>{children}</Canvas>
            </Box>
          </ScrubberProvider>
