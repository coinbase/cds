...
/**
 * Adapted from https://github.com/pbeshai/d3-interpolate-path/tree/293005eccefffaeff7b132a7c49ebd6f5e187c0f
 */
...
type TypeMap = Record<string, Array<keyof PathCommand>>;
/**
...
function convertToSameType(aCommand: PathCommand, bCommand: PathCommand): PathCommand {
  const conversionMap: Partial<Record<keyof PathCommand, keyof PathCommand>> = {
    x1: 'x',
    y1: 'y',
    x2: 'x',
    y2: 'y',
  ...
  const readFromBKeys: Array<keyof PathCommand> = ['xAxisRotation', 'largeArcFlag', 'sweepFlag'];

  // convert (but ignore M types)
  if (aCommand.type !== bCommand.type && bCommand.type.toUpperCase() !== 'M') {
    const aConverted: Partial<PathCommand> = {};
    Object.keys(bCommand).forEach((rawBKey) => {
      const bKey = rawBKey as keyof PathCommand;
      ...
      if (aValue === undefined) {
        if (readFromBKeys.includes(bKey)) {
          ...
          const mappedKey = conversionMap[bKey];
          if (aValue === undefined && mappedKey) {
            aValue = aCommand[mappedKey];
          }
...
function extend(
  ...
  const extended = countPointsPerSegment.reduce(
    (extendedCommands: PathCommand[], segmentCount: number, i: number) => {
...
export function pathCommandsFromString(d: string | null | undefined): PathCommand[] {
  ...
  let commandArgs: Array<keyof PathCommand> | undefined;
  ...
  for (let i = 0; i < tokens.length; ++i) {
    ...
    if (commandArgs) {
      command = {
        type: tokens[i] as PathCommand['type'],
      };

      // add each of the expected args for this command:
      for (let a = 0; a < commandArgs.length; ++a) {
        command[commandArgs[a]] = +tokens[i + a + 1] as number;
      }
  ...
  return function pathCommandInterpolator(t) {
    ...
    for (let i = 0; i < interpolatedCommands.length; ++i) {
      ...
      for (const arg of typeMap[interpolatedCommand.type]) {
        const aValue = (aCommand[arg] ?? 0) as number;
        const bValue = (bCommand[arg] ?? 0) as number;
        interpolatedCommand[arg] = (1 - t) * aValue + t * bValue;

        // do not use floats for flags (#27), round to integer
        if (arg === 'largeArcFlag' || arg === 'sweepFlag') {
          interpolatedCommand[arg] = Math.round((interpolatedCommand[arg] ?? 0) as number);
        }
      }
    }
 ...
 */
export default function interpolatePath(
