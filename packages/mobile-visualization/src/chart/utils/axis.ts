...
export const getAxisScale = ({
  ...
  if (!isValidBounds(adjustedDomain))
    throw new Error(
      'Invalid domain bounds. See https://cds.cbhq.net/components/charts/XAxis/#domain',
    );
...
export const getAxisConfig = (
  ...
  if (Array.isArray(axes)) {
    ...
    if (axesLength > 1 && axes.some(({ id }) => id === undefined)) {
      throw new Error(
        'When defining multiple axes, each must have a unique id. See https://cds.cbhq.net/components/charts/YAxis/#multiple-y-axes.',
      );
    }
