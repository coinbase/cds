...
import { Box, type BoxBaseProps, type BoxProps } from '@cbhq/cds-web/layout';
  ...
  forwardRef<SVGSVGElement, CartesianChartProps>(
    (
      ...
      if (layout === 'horizontal' && yAxisConfig.length > 1) {
        throw new Error(
          'When layout="horizontal", only one y-axis is supported. See https://cds.cbhq.net/components/charts/CartesianChart.',
        );
      }

      if (layout !== 'horizontal' && xAxisConfig.length > 1) {
        throw new Error(
          'Multiple x-axes are only supported when layout="horizontal". See https://cds.cbhq.net/components/charts/CartesianChart.',
        );
      }
